...
static function buildImageVerify($length=4, $mode=1, $type='gif', $width=158, $height=40, $verifyName='verify') {
  import('@.ORG.String');
 $fontPath="wifi/Common/elephant.ttf";
...
//$backColor = imagecolorallocate($im, 10,20,30); //背景色（随机）
 $borderColor = imagecolorallocate($im, 100, 100, 100); //边框色
 $pointColor = imagecolorallocate($im, mt_rand(0, 255), mt_rand(0, 255), mt_rand(0, 255)); //点颜色
...
@imagefilledrectangle($im, 0, 0, $width - 1, $height - 1, $backColor);
 ...
 Image::output($im, $type);
     ...
     * @return string
      +----------------------------------------------------------
     */
    static function buildImageVerify2($length=4, $mode=1, $type='png', $width=48, $height=22, $verifyName='verify') {
        import('@.ORG.String');
