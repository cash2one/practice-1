...
class Page extends Think {
    ...
    protected $rollPage;
	// 分页显示定制
    protected $config  =	array('prev'=>'首页','next'=>'下一页','first'=>'<<','last'=>'>>','theme'=>'%first%  %prevPage%  %linkPage%  %nextPage% %last%');
    ...
    public function show($para) {
		...
		if($this->nowPage==1){
			...
			$prevPage="<li><a href='javascript:void(0)' class='tips'>{$prevtext}</a></li>";
		}else{
			$firstPage="<li><a href='{$pageUrl}=1' class='tips'>{$firsttext}</a></li>";
			$prevPage="<li><a href='{$pageUrl}={$upRow}' class='tips'>{$prevtext}</a></li>";
		}
		...
		if($this->totalPages<=10)
		 {
			...
			{
				$currnt=($i==$this->nowPage)?" class='active'":'';
				$linkPage.="<li{$currnt}><a href='{$pageUrl}={$i}'>$i</a></li>" ;
			}
		 }else	//10页以上
		 {
			if($this->nowPage<3)  //当前页小于3
		    {
	             ...
	             $linkPage.="<li><span class='dot'>……</span></li>";
				 ...
				 for($i=$this->totalPages-3+1;$i<=$this->totalPages;$i++)
				 {
					$linkPage.="<li><a href='{$pageUrl}={$i}'>{$i}</a></li>" ;
				 }
		...
		*/
		...
		{	
		    $nextPage="<li><a href='javascript:void(0)' class='tips'>$nexttext</a></li>" ;
			//$lastPage="<a href='javascript:void(0)' class='tips'>$lasttext</a>";
		}else{
		 	$nextPage="<li><a href='{$pageUrl}=".$downRow."' class='tips'>$nexttext</a></li>";
