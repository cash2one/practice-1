<?php
function signnum($id,$ty)
{
	$arc=M("info")->where("id='$id'")->find();
	$s1=(int)$arc['s1'];
	$s2=M("order_goods")->where("pid='".$arc['id']."' and del='0'")->sum("num");
	$s3=$s1-$s2;	
	if ($s2>$s1)
	{
		$s2=$s1;	
...
function gtb($id,$tb,$field,$mkey='id')
{
	return M($tb)->where("$mkey='$id'")->getField($field);	
}
/*字段值加1*/
...
function g_f1fx($id)
{
	$sql="uid='$id' and audit='1' and CONCAT(',', fidpath, ',') like '%,1,%'";
...
{		
	...
	if($user_id!='')
	{
		...
		if ((int)$u['state']==0)
		{
			...
			exit('<script type="text/javascript">location.href="/login/";</script>');
		}
	}
	else
	{
...
{		
	...
	if ($url!=''){return $url;}
	$url=M("lm")->where("id='$id'")->getField("weburl");
	return $url;
	return "/?lm=$id";
	return "/list/$id/";	
}
...
{	
	...
	if ($url!=''){return $url;}
	$url=M("info")->where("id='$id'")->getField("weburl");
	return $url;
	return "/?id=$id";
	return "/view/$id/";
}
...
function arclist($fid,$limit='12',$cid=0,$ty=1,$order='sort asc,id desc'){
	$m=M('info');
	$sql=" CONCAT(',', fidpath, ',') like '%,".$fid.",%'";
	if ($cid>0)
	{
		//$sql.=" and cid='$cid'";	
	}
	switch ($ty)
	{
		...
		case 19:
			...
			break;
		case 11:
			$order='hit desc,settop desc,sort asc,id desc';
			break;
		case 12:
			$sql.=" and flag like '%y%'";
			break;
		case 13:
			$order='settop desc,addtime desc,sort asc,id desc';
			break;	
		case 15:
			$order='settop desc,addtime2 desc,sort asc,id desc';
			break;	
		case 6:
			$order='s6 desc,sort asc,id desc';
			break;
	}
	$newlist=$m->where($sql." and audit='1'")->order($order)->limit($limit)->select();
	//echo $m->GetLastSql();
	return $newlist;	
}
...
function isMobilex($num) {
	if (!$num) {return false;}
	return preg_match('#^13[\d]{9}$|14^[0-9]\d{8}|^15[0-9]\d{8}$|^16[0-9]\d{8}$|^17[0-9]\d{8}$|^19[0-9]\d{8}$|^18[0-9]\d{8}$#', $num) ? true : false;
}
?>