...
class Lists extends Comm
{
    ...
    public function save()
    {
        ...
        $fidpath=$lminfo['fidpath'].','.$fid.','.$fid;
        ...
        /**********图片压缩处理开始 ********/
        $toW=1200;$toH=2800;$toW2=300;$toH2=200;
        /*默认图片压缩处理 限制宽度1200 高度4800 单位px*/
        //处理banner图片压缩尺寸限制
        if ($fid==9){$toW=1980;$toH=2800;}
        ...
        if ($vo)
        {
            ...
            else
            {
                if ($vo['settop']==0)
                {
                    $data['settop_time']=time();
                }
            }
            $arc->where('id='.$id.'')->update($data); 
            if ($blm==13){$this->gxty1($id,13);}
            if ($blm==2){$this->gxty1($id,2);}
            $from=$_POST['from']; 
            msg(1,"编辑成功",$from);
        }
        else
        {
            ...
            if ($blm==13){$this->gxty1($lastInsId,13);}
            if ($blm==2){$this->gxty1($lastInsId,2);}
            $assign=$this->assign;
    ...
    /*id infoid  fid 如果是服务项目则更新团队 如果是团队则更新服务项目*/
    public function gxty1($id,$blm)
    {
        $mtb=M('info');
        $where="id='$id'";
        $vo=$mtb->where($where)->field('id,bz6,bz7')->find();
        if ($vo)
        {
            switch ($blm)
            {
                case 13:
                    ...
                    /*首先更新未关联的团队人员 取消该服务的关联*/
                    $q="CONCAT(',',fidpath,',') like '%,2,%' and CONCAT(',',bz7,',') like '%,$id,%' and id not in($idstr)";
                    $arr2=$mtb->where($q)->field('id,bz7')->select();
                    foreach ($arr2 as $ky)
                    {
                        $bz7=$ky['bz7'];
                        if ($bz7!='')
                        {
                            ...
                            $data7['bz7']=implode(',',$new7);
                            $mtb->where("id='".$ky['id']."'")->update($data7);
                        }
                    }
                    /*更新已关联技师 同时预约服务没有改服务的增加改服务信息*/
                    $q="CONCAT(',',fidpath,',') like '%,2,%' and CONCAT(',',bz7,',') not like '%,$id,%' and id in($idstr)";
                    $arr2=$mtb->where($q)->field('id,bz7')->select();
                    foreach ($arr2 as $ky)
                    {
                        $bz7=$ky['bz7'];
                ...
                case 2:
                    ...
                    /*首先更新未关联的预约服务 取消该服务的关联*/
                    $q="CONCAT(',',fidpath,',') like '%,13,%' and CONCAT(',',bz6,',') like '%,$id,%' and id not in($ids)";
                    $arr2=$mtb->where($q)->field('id,bz6')->select();
                    foreach ($arr2 as $ky)
                    {
                        $bz6=$ky['bz6'];
                        if ($bz6!='')
                        {
                            ...
                            $data6['bz6']=implode(',',$new6);
                            $mtb->where("id='".$ky['id']."'")->update($data6);
                        }
                    }
                    /*更新已关联技师 同时预约服务没有改服务的增加改服务信息*/
                    $q="CONCAT(',',fidpath,',') like '%,13,%' and CONCAT(',',bz6,',') not like '%,$id,%' and id in($ids)";
                    $arr2=$mtb->where($q)->field('id,bz6')->select();
                    foreach ($arr2 as $ky)
                    {
                        $bz7=$ky['bz6'];
