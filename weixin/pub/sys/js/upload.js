		...
		$(this).simpleUpload(urls, {
			...
			success: function(data){
				...
				if (data.success) {
					...
					var nh=$('#'+objx).html();
					nh=nh+'<li><img src="'+fileurl+'" /><div><a href="#no" class="rt">删除图片</a></div></li>';
		...
		$(this).simpleUpload(urls, {
			...
			success: function(data){
				...
				if (data.success) {
					...
					var objx=$(obj).attr("toid");
					var nh=$('#'+objx).html();
					nh=nh+'<li><img src="'+fileurl+'" /><div><a href="#no" class="rt">删除图片</a></div><input type="hidden" name="'+selects+'[]" value="'+fileurl+'"></li>';
					$('#'+objx).html(nh);show_('上传成功');
				} else {show_('上传失败：'+data.message);}
			},
			error: function(error){show_('上传失败：'+error.message);}
