...
K.options = {
	...
	items : [
		...
		'insertvideo', 'media', 'insertfile', 'table', 'hr', 'emoticons', 'baidumap', 'pagebreak',
		'anchor', 'link', 'unlink','audio'
	],
	...
	htmlTags : {
		...
		a : ['id', 'class', 'href', 'target', 'name','style'],
		...
		video : ['src','controls','class','source','poster' ,'preload','data-src','width','height'],
		source : ['src','controls','class','type'],
...
function _toRange(rng) {
	...
	try{
		...
		doc = startContainer.getOwnerDocument() || startContainer;
	}
	range = new KRange(doc);
...
_extend(KCmd, {
	...
	insertinsertvideo   : function(url, title, width, height, border, align) {
		...
		var html = '<p'+title+' class="qmvideo"'+align+'><video width="'+width+'" height="'+height+'" controls="controls" data-src="'+_escape(url)+'">';
			html += '<source src="'+_escape(url)+'" type="video/mp4">'
			html += '</video></p>';
		...
		function ready() {
			...
			if (_WEBKIT) {
				K(doc).click(function(e) {
					if (K(e.target).name === 'img' || K(e.target).name === 'video') {
						cmd.selection(true);
