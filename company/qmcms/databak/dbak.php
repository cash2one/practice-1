...
@ob_end_flush();
    ...
    private function _write_file($sql, $filename, $dir) {
		...
		$files_to_zip = array( $dir2 . $filename); 
        return $re;
    }
    ...
    function restore($sqlfile) {
        if (! file_exists ( $sqlfile )) {
            $this->_showMsg("sql文件不存在！请检查",true);exit ();
        }
    ...
    private function close() { mysql_close ( $this->db );}
...
$dbarry=include("../app/database.php");
	
$sq_charset='utf8';
$sq_name=$dbarry['hostname'];
$sq_username=$dbarry['username'];
$sq_password=$dbarry['password'];
$sq_database=$dbarry['database'];
